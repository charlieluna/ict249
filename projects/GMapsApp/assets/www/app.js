...
function fetchDetails(entry) {
  ...
  var detailsUrl = "https://maps.googleapis.com/maps/api/place/details/json?reference=" + entry.reference + "&sensor=true&key=AIzaSyApZUme2o9nlqwirFOxzIdGY3HyNGxmqtA";
  $("#name").html("");
  $("#address").html("");
  $("#phone").html("");
  $("#rating").html("");
...
function initiateMap() {
  $("#map").on("pagebeforecreate", function () {
    try {
      $('#map_canvas').gmap({
        'center': mapdata,
        'zoom': 12,
...
function initiateFavorites() {
  $("#fav").on("pagebeforeshow", function () {
  ...
  $("#search").click(function () {
    try {
      ...
      navigator.geolocation.getCurrentPosition(function (position) {
        ...
        var url = "https://maps.googleapis.com/maps/api/place/search/json?location=" + position.coords.latitude + "," + position.coords.longitude + "&radius=" + radius + "&name=" + $("#searchbox").val() + "&sensor=true&key=AIzaSyApZUme2o9nlqwirFOxzIdGY3HyNGxmqtA";
        console.log("url: " + url)
